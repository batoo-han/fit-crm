...
4. **Проверка загрузок**: Убедитесь, что директория `/opt/fitness-crm/uploads` существует и доступна для записи
   - При каждом запуске система автоматически очищает неиспользуемые файлы из `uploads/`
   - Удаляются файлы, на которые нет ссылок в БД
   - Удаляются битые ссылки из БД, если файла больше нет
   - Логотипы сохраняются с понятными именами: `logo_widget.<расширение>`, `logo_site.<расширение>`
   - PDF-файлы сохраняются с осмысленными именами: `<id_клиента>_<недель>_weeks_<дата>_<номер>.pdf`
