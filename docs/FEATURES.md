...
- Что делает:
  ...
  - Шаблоны программ для PDF-футтеров с плейсхолдерами (редактируемые в админке).
  - Очистка программ: архивация отправленных, удаление неиспользуемых.
  - Осмысленные имена PDF-файлов: `<id_клиента>_<недель>_weeks_<дата>_<номер>.pdf`.
- Сценарии:
  ...
  - Настроить шаблон футтера для PDF в разделе «Шаблоны программ».
  - Выполнить очистку старых программ через раздел «Очистка программ».

## 6) Платежи и статусы
- Что делает:
  - Поддержка YooKassa и Tinkoff, вебхуки и периодические проверки статусов.
  ...
  - Настройки платёжных систем с кнопкой сохранения (все изменения применяются по кнопке «Сохранить настройки»).
  - Автоприменение промокода из списка активных промокодов (настраивается в разделе «Интеграции»).
- Сценарии:
  ...
  - Админ настраивает платёжные системы: выбирает провайдера, вводит ключи, нажимает «Сохранить настройки».
  - Админ выбирает промокод для автоприменения из списка активных промокодов.
...
- Что делает:
  ...
  - Понятные имена файлов: логотипы сохраняются как `logo_widget.<расширение>` и `logo_site.<расширение>`.
  - Автоматическая очистка неиспользуемых файлов при запуске системы.
- Сценарии:
  - Админ загружает логотип из файла (компьютер/телефон), поле URL заполняется автоматически.
  - При каждом запуске система автоматически удаляет файлы из `uploads/`, на которые нет ссылок в БД, и очищает битые ссылки.
