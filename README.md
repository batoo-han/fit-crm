...
- **Админ-панель (React)**: настройка сайта, виджета чата, контента, визуальных тем, управление клиентами и программами, шаблоны программ, очистка старых данных.
- **Telegram-бот**: выдача программ, FAQ, коммуникация с клиентами.
- **Интеграции**: уведомления в Telegram, гибкое подключение LLM-провайдеров, настройка CORS для локального и продакшн окружений, автоматическая очистка неиспользуемых файлов.
...
- Интеграция с YooKassa/Tinkoff: код/скидка попадают в платежи, учёт использования при успешной оплате.
- UI в разделе `Маркетинг` для управления промокодами и проверки вручную.
- Автоприменение промокода: выбор из списка активных промокодов в настройках платёжных систем.
...
- Настройки платёжных систем сохраняются по кнопке «Сохранить настройки» (все изменения применяются только после сохранения).
